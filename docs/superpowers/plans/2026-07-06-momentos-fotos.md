...
### Task 6: Remove the dead album-based photo code

Once content is served from `photos.featured`/`photos.items` (Task 1) and the bento `PhotoGallery` exists (Task 2), the album-based `Gallery` component and the `Album` type are no longer backed by any real content — they survive only as a styleguide demo. Remove them and repoint the styleguide at `PhotoGallery`.

**Files:**
- Modify: `src/styleguide/StyleGuide.tsx`
- Delete: `src/components/Gallery.tsx`
- Delete: `src/components/Gallery.test.tsx`
- Modify: `src/content/types.ts` (remove the `Album` interface)

**Interfaces:**
- Consumes: `PhotoGallery` from `src/components/sections/PhotoGallery.tsx` (Task 2); `Photo` from `src/content/types.ts`.
- Produces: no album-based photo code remains; `Album`, `Gallery`, and `sampleAlbums` are gone from the codebase.

- [ ] **Step 1: Repoint the styleguide gallery demo at `PhotoGallery`**

In `src/styleguide/StyleGuide.tsx`:

Change the type import (line 1) from:
...
Change the Gallery import (line 8) from:
...
Replace the `sampleAlbums` constant:
...
- [ ] **Step 2: Delete the album-based Gallery component and its test**

Run:

```bash
git rm src/components/Gallery.tsx src/components/Gallery.test.tsx
```

- [ ] **Step 3: Remove the `Album` interface from types**

In `src/content/types.ts`, delete the `Album` interface (keep `Photo`, which is still used by `Lightbox`, `PhotoGallery`, and the momentos selector):
...
- [ ] **Step 4: Verify no album-based references remain**

Run: `grep -rn "Album\|components/Gallery\|sampleAlbums" src`
Expected: no matches (only `VideoGallery`/`PhotoGallery` may appear, which are unrelated).

- [ ] **Step 5: Type-check, lint, and test**

Run: `pnpm build && pnpm lint && pnpm test`
Expected: all succeed (styleguide compiles against `PhotoGallery`; no dangling `Album`/`Gallery` references).

- [ ] **Step 6: Commit**

```bash
git add src/styleguide/StyleGuide.tsx src/content/types.ts
git commit -m "refactor: remove dead album-based Gallery in favor of PhotoGallery"
```
...
- The album-based `src/components/Gallery.tsx` and the `Album` type become dead once the new `featured`/`items` structure lands; Task 6 removes them and repoints the styleguide demo at `PhotoGallery`.
