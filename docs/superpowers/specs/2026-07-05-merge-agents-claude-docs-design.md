# Merge AGENTS.md and CLAUDE.md Design Document

**Goal:** Consolidate AI Agent instructions from `CLAUDE.md` and `AGENTS.md` under `AGENTS.md`, and update `CLAUDE.md` to reference `AGENTS.md` like `GEMINI.md` and `JULES.md`.

**Architecture:**
- `AGENTS.md` will serve as the single source of truth for repository structure, guidelines, conventions, stack, and commands.
- `CLAUDE.md` will be converted to a clean reference pointer file using the standard imported formatting.
...
We will rewrite `AGENTS.md` to contain all the unified information:
...
## Verification Plan
1. Check that `AGENTS.md` contains all content.
2. Check that `CLAUDE.md` has the new pointer content.
3. Run `pnpm lint` and `pnpm format` to make sure formatting is intact.
